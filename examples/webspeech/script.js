    ...
    if (this.state.error) {
      return (
        <div style={{background: 'yellow', padding: '0.75em', overflow: 'hidden'}}>
          <strong style={{float: 'left', fontSize: '2em', marginRight: '0.5em'}}>:‘(</strong>
          <span>
            It looks like your browser does not support the <a href="https://developer.mozilla.org/en-US/docs/Web/API/SpeechRecognition">SpeechRecognition</a> API.
          </span>
        </div>
      );
    }
    ...
    return (
      <div>
        ...
        <span>Everything heard: </span>
          <ul>{this.state.heard.map((h, i) => <li key={i}>{h}</li>)}</ul>
        </div>
      </div>
    );
  }
